...
const defaultPreviewTexts = {
    "Auto": "Hello, I'm delighted to assist you with our voice services.",
    ...
    "Chinese": "您好,很高兴能为您提供配音服务。选择您感兴趣的音色,让我们一起开启声音创作的奇幻之旅吧。",
    ...
    "Filipino": "Kumusta, natutuwa akong tulungan ka sa aming mga serbisyo sa boses. Pumili ng boses na akma sa iyo, at magsimula tayo ng ating malikhaing paglalakbay sa audio.",
    ...
    "Tamil": "வணக்கம், எங்கள் குரல் சேவைகளில் உங்களுக்கு உதவ மகிழ்ச்சியாக உள்ளது.",
...
function createClone() {
    ...
    // Tự động lấy preview text từ ngôn ngữ đã chọn
    const finalPreviewText = defaultPreviewTexts[selectedLanguage] || defaultPreviewTexts["English"];
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            ...
            if (status === 'timeout') {
                showToast('error', '⏱️ Quá thời gian chờ (Timeout).');
...
function loadClonedVoices() {
    $.post('../../ajaxs/voice_cloning2.php', { action: 'list_clones' }, function(res) {
        if (res.status === 'success') {
...
function renderGrid(list) {
    ...
    list.forEach(v => {
        ...
        if (tagsHtml === '' && v.tags && Array.isArray(v.tags)) {
            tagsHtml = v.tags.map(t => `<span class="vc-tag">${t}</span>`).join('');
        }
