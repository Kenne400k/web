...
// ========== CHECK MAINTENANCE MODE (SERVER 2) ==========
...
if (isset($mysqli)) {
    $stmt_check = $mysqli->prepare("
        ...
        WHERE service_path IN (
            '/ai/tts2?provider=elevenlabs',  -- 🔥 Đã đổi thành tts2
            '/ai/tts2?provider=minimax',     -- 🔥 Đã đổi thành tts2
            '/ai/genai_backup'               -- Giữ nguyên backup chung
        )
    ");
    ...
    if ($stmt_check) {
        ...
        while ($row = $res->fetch_assoc()) {
            if ($row['is_active'] == 1) {
                // 🔥 Check đúng path của Server 2
                if ($row['service_path'] === '/ai/tts2?provider=elevenlabs') $elevenlabs_down = true;
                if ($row['service_path'] === '/ai/tts2?provider=minimax') $minimax_down = true; 
...
// Nếu cả 3 đều sập thì chuyển hướng
if ($elevenlabs_down && $minimax_down && $genai_backup_down) {
    header("Location: /pages/maintenance.php?path=" . urlencode('/ai/tts2'));
    exit(); 
}

// ========== HELPER: PARSE CREDITS WITH MULTIPLIERS ==========
/**
 * Parse credit string from database
 * Supports: "5000000", "5m" (5 million), "5k" (5 thousand)
 * @param mixed $value Credit value from database
 * @return int Parsed credit amount
 */
function parseCredits($value) {
    ...
    if (preg_match('/^([\d.]+)([km]?)$/', $value, $matches)) {
...
if (isset($_SESSION['Users'])) {
    ...
    if (isset($mysqli)) {
    // ✅ SỬA 1: Thêm apikey2 vào SELECT
    $stmt = $mysqli->prepare("
        SELECT id, apikey, apikey2, credits2
        FROM Users 
    ...
    if ($res) {
        $user_id_real = $res['id'];
        $current_credits = parseCredits($res['credits2']);
        
        // ✅ SỬA 2: Check cả apikey HOẶC apikey2
        if (!empty($res['apikey']) || !empty($res['apikey2'])) {
            $has_api_key = true;
        }
    }
    $stmt->close();
                ...
                <div id="emptyState" class="empty-state">
                    ...
                    <div class="es-tips" id="emptyTips">
                        ...
                        <div class="es-tip">
                            ...
                            <span>📁 <?php echo $t['tip_drag_drop']; ?> <b>.txt</b> <?php echo $t['tip_here']; ?></span>
                        </div>
                    </div>
                </div>
            </div>
...
<div style="position: relative; display: inline-block;">
    ...
    ">
        ...
        <!-- Provider Name -->
        <div id="tooltipProvider" style="font-size: 12px; color: #667eea; margin-bottom: 6px; font-weight: 600;">
            • Provider: <span id="tooltipProviderName">-</span>
        </div>
        
        <!-- Model Fee -->
        <div id="tooltipModel" style="font-size: 12px; color: #ccc; margin-bottom: 4px;">
            • Model: <span style="color: #fff; font-weight: 600;" id="tooltipModelFee">x1.0</span>
        </div>
        
        <!-- Voice Fee (Minimax only) -->
        <div id="tooltipVoice" style="font-size: 12px; color: #ccc; margin-bottom: 4px; display: none;">
            • Voice: <span style="color: #fff; font-weight: 600;" id="tooltipVoiceFee">x1.0</span>
        </div>
        
        <!-- SRT Fee -->
        <div id="tooltipSrt" style="font-size: 12px; color: #ccc; display: none;">
            • Auto SRT: <span style="color: #fff; font-weight: 600;">x1.15</span>
        </div>
        ...
        <div style="font-size: 11px; color: #888; text-align: center;">
            <span id="tooltipFormula">Ký tự × Model × SRT</span>
        </div>
    </div>
</div>
...
<div id="elevenlabs-cost-ui" style="display: none;">
    <div class="toggle-row" style="display: flex; justify-content: space-between; align-items: center; padding-top: 20px; border-top: 1px solid #222; margin-top: 10px;">
        ...
        <!-- Delay Button -->
        <div style="position: relative; display: inline-block;">
            <button class="reset-btn" id="btnDelayEleven" onclick="showDelayTooltip(event, 'elevenlabs')" style="width: auto; margin: 0; padding: 8px 14px; background: #1a1a1a; border-color: #333;">
...
<div id="minimax-cost-ui" style="display: none;">
    <div class="toggle-row" style="display: flex; justify-content: space-between; align-items: center; padding-top: 20px; border-top: 1px solid #222; margin-top: 10px;">
        ...
        <!-- Delay Button -->
        <div style="position: relative; display: inline-block;">
            <button class="reset-btn" id="btnDelayMinimax" onclick="showDelayTooltip(event, 'minimax')" style="width: auto; margin: 0; padding: 8px 14px; background: #1a1a1a; border-color: #333;">
                <i class="bi bi-pause-circle"></i> <?php echo $lang === 'vi' ? 'Khoảng dừng' : 'Delay'; ?>
            </button>
        </div>
        <button class="reset-btn" onclick="resetCurrentSettings()" style="width: auto; margin: 0; padding: 8px 14px; background: #1a1a1a; border-color: #333;">
...
<div id="voiceModal" class="voice-modal">
    ...
    <div class="custom-dropdown" id="sortDropdown" style="display: none; min-width: 170px;">
        <button class="dropdown-btn" onclick="toggleSortDropdown(event)" style="height: 42px; background: #1a1a1a; border: 1px solid #333; color: #fff; display: flex; align-items: center; justify-content: space-between; width: 100%; padding: 0 12px; border-radius: 8px; cursor: pointer;">
            ...
            <i class="bi bi-chevron-down dropdown-arrow" style="font-size: 10px; color: #666;"></i>
        </button>
    ...
    <div class="bulk-modal-content">
        ...
        <div class="bulk-modal-body" id="bulkModalScrollArea">
            <div class="bulk-drop-zone" id="bulkDropZone">
                ...
                <p><?php echo $lang === 'vi' 
                    ? 'Hỗ trợ: .txt, .zip (tối đa 20 file, mỗi file < 5MB)' 
                    : 'Supports: .txt, .zip (max 20 files, each < 5MB)'; ?></p>
            </div>
 <input type="file" id="bulkFileInput" multiple accept=".txt,.zip" style="display: none;">

            <div class="bulk-file-list" id="bulkFileList" style="margin-top: 20px; display: none;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
        <h5 style="margin: 0; color: #fff; font-size: 14px;">
            Danh sách file (<span id="fileCount">0</span>)
        </h5>
        
        <div style="display: flex; gap: 8px;">
            <button onclick="$('#bulkFileInput').click()" style="
                background: #222; border: 1px solid #444; color: #fff;
                padding: 6px 12px; border-radius: 6px; font-size: 12px; cursor: pointer;
                display: flex; align-items: center; gap: 6px; transition: 0.2s;
            " onmouseover="this.style.background='#333'" onmouseout="this.style.background='#222'">
                <i class="bi bi-plus-lg"></i> Thêm tệp
            </button>

            <button class="bulk-btn-clear" onclick="clearAllFiles()" style="
    ...
    <div class="bulk-file-list-items" id="fileListContainer" style="
        max-height: 250px; overflow-y: auto; border: 1px solid #222;
...
<div id="folderConfirmPopup" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.85); backdrop-filter: blur(5px); z-index: 99999; align-items: center; justify-content: center;">
    <div style="background: #111; border: 1px solid #333; border-radius: 16px; padding: 32px; max-width: 450px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.9); text-align: center;">
        ...
        <h3 style="margin: 0 0 12px 0; font-size: 18px; font-weight: 700; color: #fff;">
            Bạn muốn tải <span id="folderFileCount" style="color: #667eea;">0</span> tệp?
        </h3>
        
        <p style="color: #999; font-size: 14px; margin-bottom: 24px; line-height: 1.6;">
            Thao tác này sẽ thêm tất cả các tệp hợp lệ vào danh sách chờ xử lý.
        </p>
        
        <div id="folderFileList" style="max-height: 150px; overflow-y: auto; margin-bottom: 24px; text-align: left; background: #0a0a0a; border: 1px solid #222; border-radius: 8px; padding: 10px;"></div>
        
        <div style="display: flex; gap: 12px;">
            <button id="folderCancelBtn" onclick="$('#folderConfirmPopup').fadeOut()" style="flex: 1; padding: 12px; border-radius: 10px; border: 1px solid #333; background: transparent; color: #ccc; font-weight: 600; cursor: pointer;">Huỷ</button>
            <button id="folderUploadBtn" style="flex: 1; padding: 12px; border-radius: 10px; border: none; background: #fff; color: #000; font-weight: 700; cursor: pointer;">Tải lên ngay</button>
        </div>
    </div>
</div>
...
<div id="bulkConfirmPopup" class="bulk-confirm-overlay">
    <div class="bulk-confirm-box">
        ...
        <div class="bc-header">
            ...
            <h3>Xác nhận xử lý hàng loạt</h3>
        </div>
        ...
        <div class="bc-body">
            ...
            <div class="bc-info-row">
                ...
                <span class="bc-value" id="bcFileCount">0</span>
            </div>
            ...
            <div class="bc-info-row">
                ...
                <span class="bc-value" id="bcCharCount">0</span>
            </div>
            ...
            <div class="bc-info-row">
                ...
                <span class="bc-value bc-cost" id="bcCost">0 credits</span>
            </div>
            ...
            <div class="bc-info-row">
                ...
                <span class="bc-value balance" id="bcBalanceAfter">0 credits</span>
            </div>
            
            <!-- Warning (hidden by default) -->
            <div id="bcWarning" class="bc-warning" style="display: none;">
                <i class="bi bi-exclamation-circle"></i>
                <span id="bcWarningText">Số dư không đủ!</span>
            </div>
        </div>
        
        <!-- Footer -->
        <div class="bc-footer">
            <button class="bc-btn bc-btn-cancel" onclick="closeBulkConfirmPopup()">
                <i class="bi bi-x-lg"></i>
                Hủy bỏ
            </button>
            <button class="bc-btn bc-btn-confirm" id="btnBulkConfirm" onclick="confirmBulkProcess()">
...
<style>/* ========================================
   🔥 FIX BULK CONFIRM POPUP CSS
   Thay thế CSS cũ trong tts2.php
======================================== */
...
.bulk-confirm-overlay {
    ...
    backdrop-filter: blur(8px);
    display: flex;  /* ✅ Luôn dùng flex để căn giữa */
    align-items: center;
    justify-content: center;
    z-index: 99999;
    
    /* 🔥 ẨN bằng opacity + visibility thay vì display:none */
    opacity: 0;
    visibility: hidden;
    pointer-events: none;
...
.bulk-confirm-overlay.show {
    opacity: 1 !important;           /* 🔥 QUAN TRỌNG: !important */
    visibility: visible !important;   /* 🔥 QUAN TRỌNG: !important */
...
.bc-btn-confirm.loading {
    pointer-events: none;
}

.bc-btn-confirm.loading::before {
    content: "";
    width: 14px;
    height: 14px;
    ...
    <div class="delete-modal-box">
        ...
        <div class="dm-footer">
            ...
            <button class="btn-confirm-delete" id="btnConfirmDelete">
                <i class="bi bi-trash"></i> <span id="btnDeleteText">Xóa</span>
            </button>
        </div>
    </div>
</div>
...
<!-- ========== POPUP REMAKE TASK - SUPER SMOOTH ========== -->
<div id="remakeTaskModal" class="custom-modal-overlay" style="display: none;">
    <div class="remake-modal-box">
        ...
        <!-- 🔥 [MỚI] TOGGLE XUẤT PHỤ ĐỀ -->
        <div class="remake-subtitle-toggle">
            <div class="toggle-label">
                <i class="bi bi-file-earmark-text"></i>
                <span><?php echo $lang === 'vi' ? 'Xuất phụ đề (SRT)' : 'Export Subtitle (SRT)'; ?></span>
                <span class="toggle-badge">+15%</span>
            </div>
            <label class="toggle-switch">
...
<div id="delayTooltip" class="delay-tooltip" style="display: none;">
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px;">
        <span style="font-size: 13px; font-weight: 600; color: #fff;"><?php echo $lang === 'vi' ? 'Chèn khoảng dừng' : 'Insert Delay'; ?></span>
        <i class="bi bi-x-lg" onclick="hideDelayTooltip()" style="cursor: pointer; color: #888; font-size: 14px;"></i>
    </div>
    <div style="display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 12px;">
        <button class="delay-quick-btn" onclick="insertDelay(0.3)">0.3s</button>
        <button class="delay-quick-btn" onclick="insertDelay(0.5)">0.5s</button>
        <button class="delay-quick-btn" onclick="insertDelay(1)">1s</button>
        <button class="delay-quick-btn" onclick="insertDelay(2)">2s</button>
        <button class="delay-quick-btn" onclick="insertDelay(3)">3s</button>
    </div>
    <div style="display: flex; align-items: center; gap: 8px;">
        <input type="number" id="customDelayInput" min="0.1" max="10" step="0.1" value="0.5"
            style="flex: 1; padding: 6px 10px; background: #1a1a1a; border: 1px solid #444; border-radius: 6px; color: #fff; font-size: 13px;">
        <span style="color: #888; font-size: 12px;">s</span>
        <button class="delay-add-btn" onclick="insertDelay(parseFloat($('#customDelayInput').val()))"><?php echo $lang === 'vi' ? 'Thêm' : 'Add'; ?></button>
    </div>
    ...
    ">
        ...
        <div style="padding: 24px 28px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 14px;">
                ...
                <div style="display: flex; gap: 14px; align-items: flex-start; background: #111; border: 1px solid #333; border-radius: 12px; padding: 16px;">
                    <div style="width: 36px; height: 36px; background: #1a1a1a; border: 1px solid #444; border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                        <i class="bi bi-lightning-charge-fill" style="font-size: 16px; color: #fff;"></i>
                    </div>
                    <div>
                        <div style="font-size: 14px; font-weight: 600; color: #fff; margin-bottom: 4px;">
...
const isElevenLabsDown = <?php echo $elevenlabs_down ? 'true' : 'false'; ?>;  // ← Thêm "is"
const isMinimaxDown = <?php echo $minimax_down ? 'true' : 'false'; ?>;        // ← Thêm "is"
const isBackupEligible = <?php echo $backup_eligible ? 'true' : 'false'; ?>;   // ← Thêm "is"
const jsLang = <?php echo json_encode($t); ?>;
</script>

<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js"></script>
<script src="/pages/AI/tts2.js?v=<?php echo time(); ?>"></script>
